...
function Cards({ filterType }) {
  ...
  const tenderStatus = useSelector((state) => state.tenders.status)
  ...
  const filteredTenders = useMemo(() => {
    ...
    result = result.filter((tender) => tender.expirationDate >= today);
  ...
  useEffect(() => {
    dispatch(fetchTenders());
    dispatch(fetchAllUsers());
  }, [dispatch, user?.bookmarked, filterType]);
  ...
  return (
    <div className={!loading ? "tenders" : "tenders load"} >
      {loading ? (<><Loading /></>) : (<>
